...
class FileDescriptor {
 public:
  ...
  ssize_t GetFileSize() { return GetFileSize(fd_); }

  // Put the file object referenced by the descriptor into blocking mode.
  bool MakeBlocking() { return SetBlockingMode(fd_, true); }

  // Put the file object referenced by the descriptor into nonblocking mode.
  bool MakeNonblocking() { return SetBlockingMode(fd_, false); }

  // Set pipe capacity (member function, delegates to static, above.)
  int SetPipeCapacity(int size) { return SetPipeCapacity(fd_, size); }
