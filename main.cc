...
int main(int argc, char *argv[]) {
  ...
  auto dest_file = std::make_shared<FileDescriptor>(
			open(argv[2], flag_bits, mode_bits));
  if (dest_file->Get() < 0) {
	...
	while (bytes_written_to_file < src_file_size) {
		...
		for (int n = 0; n < nfds; ++n) {
			if (events[n].data.fd == PIPE_WRITER) {
				printf("read from input file.\n");
				// The write side of the pipe is ready for data.
				const ssize_t xferred = splice(SOURCE_FILE, nullptr,
						PIPE_WRITER, nullptr, ONE_MEG, SPLICE_F_MOVE | SPLICE_F_NONBLOCK);
				...
				const ssize_t xferred = splice(PIPE_READER, nullptr,
						DEST_FILE, nullptr, ONE_MEG, SPLICE_F_MOVE | SPLICE_F_NONBLOCK);
